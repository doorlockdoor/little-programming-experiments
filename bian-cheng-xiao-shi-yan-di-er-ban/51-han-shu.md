...
之所以会有这样的区别，是因为在定义函数时，我们并不知道外面的使用者会如何取变量名称，随便举一个例子`def get_hp(character)`，假设这是获取角色血量的函数。

在外面使用时我们写`get_hp(character_1)`，`get_hp(character_2)`，这里的`character_1`和`character_2`就是实际参数。而无论外面使用什么名称，在函数里面都统一叫做本地名，即`character`，这就是形式参数，它只有在外面真正传递参数进来时，才会“复苏”，并指向对应的数据。
