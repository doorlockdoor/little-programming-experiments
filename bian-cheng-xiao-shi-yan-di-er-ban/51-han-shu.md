...
像前面例子中用到的max和min等内置函数，它们的def定义就写在官方内置模块中，程序运行后可以自动使用，不需要我们再重复定义一遍。而我们自己的函数（定义）则放在自己的代码中，相当于我们的本地代码库。而如果想要引用别人写好的函数（定义），还可以导入官方库或第三方库，在导入库之后，我们就可以随意使用别人写好的函数功能了，这在后文的条件语句章节有实际案例。
...
之所以会有这样的区别，是因为在定义函数时，我们并不知道外面的使用者会如何取变量名称，举一个例子`def get_hp(character)`，假设这是获取角色血量的函数。

在外面使用时我们写`get_hp(character_1)`，`get_hp(character_2)`，这里的`character_1`和`character_2`就是实际参数。而无论外面使用什么名称，在函数里面都会有另一个名字负责绑定背后的数据，相当于有两条线指向同一个目标内存，即`character`，这就是形式参数，它负责“接手”数据，并进行内部处理。值得注意的是，在不同的编程语言中，形参的设计和效果也是不一样的，因此上述说法只是便于理解的通俗描述，需要根据编程语言的不同进行修改。

在“接手”数据之后，我们又要引入一个新的概念，**作用域**。
...
局部作用域可以嵌套局部作用域，而最外层的代码则被称为全局作用域，变量也被称为全局变量，会一直存在。

> 这也是一个区分实际参数和形式参数的原因，因为形参会在函数结束后释放，而外部的实参则会维持原状，直到它自己也结束了它的作用域。

**这样设计的初衷是形成一个个的代码执行环境，每一个环境从零内存占用开始、到零内存占用结束，形成一个完整的周期。局部环境拥有自己的局部变量，也可以访问更外层的局部/全局变量，然后在环境执行完成后，释放自己所占用的内存。根据这一特性，每一个变量都有对应的生命周期，结束生命周期后即不存在**。

但一般情况下我们都是无法主动缩进（作用域）的，为了防止滥用，只有特殊的语句才会有局部作用域。
...
> 为什么函数名必须加上圆括号？一个简单的理由是区分变量名和函数名，函数名的本质只是一个名称+绑定数据，而变量名的本质也是如此，两者不存在形式上的差异，区别只在于它们绑定的数据类型。事实上，在高阶运用中，我们可以直接在变量名后面加上圆括号把它当成函数使用；也可以把函数名没有圆括号，当成变量名传递，当然，它的数据类型得是函数，不然会报错（详见第六章数据类型）。这一理念即是函数是一等公民（first class），它和变量是等价的，它们都有对应的数据类型，不过并不是所有的编程语言都支持这一特性，大家了解即可。
